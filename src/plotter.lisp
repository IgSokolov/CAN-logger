...
(defun add-canvas-obj (env plot-text-area label screen colormap plot-window)
  ...
  (let ((obj (make-canvas-obj
	      :root plot-window
	      :canvas (make-canvas plot-window (plot-text-settings-display plot-text-area) screen colormap)
	      :label label)))
	...
	(when data
	  (let ((data-list (remove-if #'null data)))
	    (let ((plot-buf-1) ;; for draw-lines
		  ...
		  (y0-mapped-list (mapcar #'(lambda (item) (map-y0-to-plot (cdr item) (plot-env-y-min env) (plot-env-y-max env) plot-window-size)) data-list))
		  (t0-mapped-list (mapcar #'(lambda (item) (round (* plot-window-size (/ (car item) t-max)))) data-list)))
  ...
  (let ((butlast-point (cdr (find-if #'consp (cdr (canvas-obj-data canvas-obj))))))
    (when butlast-point
      (draw-rectangle (plot-text-settings-labels-window plot-text-area)
		      ...
		      (+ (drawable-width (canvas-obj-root canvas-obj)) 5) ;; clean 5 pixels more to rm artifacts
...
(defun plot-pd (pd env plot-text-area screen grid plot-window t-max dt x-end dx-grid point-size x-shift data-length-max plot-window-size y-coords colormap)
  ...
  (let ((y0 (plot-data-value pd))
    ...
    (multiple-value-bind (canvas-obj rest-canvas-objs) (fetch-canvas-obj label (plot-env-canvas-obj-db env))			       
      ...
      (unless canvas-obj
	(setq canvas-obj (add-canvas-obj env plot-text-area label screen colormap plot-window)))
      (destructuring-bind (canvas-foreground canvas-background) (canvas-obj-canvas canvas-obj)
	...
	(if (recompute-y-limits y0 env) ;; check if we need rescaling (bug here? see commit 0c04f5b)
	    (progn
	      (clear-area (plot-text-settings-labels-window plot-text-area)) ;; delete labels when redraw
	    ...
	    (let ((y0-mapped (map-y0-to-plot y0 (plot-env-y-min env) (plot-env-y-max env) plot-window-size)))
	      (multiple-value-bind (n-empty-points prev-point) (find-previous-point (cdr (canvas-obj-data canvas-obj)))
		...
		(draw-arc plot-window canvas-foreground (- (- plot-window-size (/ point-size 2)) 2)
 		...
 		(when prev-point ;; is the dataset larger then ((NIL . 0.0d0)) ?	      
		  (let ((prev-point-mapped (map-y0-to-plot prev-point (plot-env-y-min env) (plot-env-y-max env) plot-window-size)))		    
		    (draw-line plot-window canvas-foreground (- plot-window-size (* x-shift (- n-empty-points (- (length rest-canvas-objs) 1)))) ;; 2 -> 0; 3 -> 1
...
(defun take-last-elements (queue)
  (let ((acc))
    (loop for data = (sb-concurrency:dequeue queue) do
      (if data
	  (push data acc)
	  (return-from take-last-elements (nreverse acc))))))
