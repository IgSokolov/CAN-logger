...
(define-condition no-suitable-parser-found (cannot-parse-item) ())
...
(defun try-parse (string parser)
  ...
  (when (null parser)
    (error 'no-suitable-parser-found :item string))
  ...
  (handler-case 
	...
	(when content
	  (let ((can-db (make-hash-table)))
	    ...
	    can-db)))
    (error (c)
      (format t "Error. Cannot parse config file: ~a~%" c)
      (sb-ext:exit))))
